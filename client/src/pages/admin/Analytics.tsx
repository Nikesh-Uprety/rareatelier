  ...
  return (
    <div className="min-h-screen bg-muted dark:bg-neutral-900 pb-20 admin-font">
      ...
      <div className="grid grid-cols-1 gap-4 px-4 sm:gap-5 sm:px-6 md:grid-cols-2 lg:grid-cols-4 xl:gap-6 xl:px-8">
        ...
        {/* Top Products + Revenue Distribution (Sync Highlight) — 3 cols */}
        <TopProductsWithSyncHighlight analytics={data} isLoading={isLoading} className="lg:col-span-3" />
...
// ─── Top Products + Revenue Distribution (Sync Highlight) ─────────────────────

function TopProductsWithSyncHighlight({ analytics, isLoading, className }: { analytics: AdminAnalytics | undefined; isLoading: boolean; className?: string }) {
  const products = analytics?.topProducts ?? [];
  const topProducts = useMemo(() => products.slice(0, 8).map(p => ({ ...p, revenue: toSafeNum(p.revenue) })), [products]);
  const totalRevenue = useMemo(() => products.reduce((sum, pr) => sum + toSafeNum(pr.revenue), 0), [products]);
  const pieColors = [COLOR_TOKENS.emerald, COLOR_TOKENS.blue, COLOR_TOKENS.purple, COLOR_TOKENS.amber, COLOR_TOKENS.red, "#06b6d4", "#f97316", "#ec4899"];
    ...
    <div className={cn(cardStyles, className)}>
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest -mt-3">Best performing items · Hover to highlight</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-8">
          ...
          {isLoading ? (
            ...
            <div className="space-y-1">
              {topProducts.map((p, idx) => {
                ...
                const isFaded = highlightedIdx !== null && !isHighlighted;
                return (
                  <div
                    key={`${p.name}-${idx}`}
                    ...
                    className={cn(
                      "flex items-center gap-4 py-3 px-3 rounded-xl cursor-pointer transition-all duration-200",
                      isHighlighted && "bg-emerald-500/5 ring-1 ring-emerald-500/20",
                      isFaded && "opacity-30",
                      !isHighlighted && !isFaded && "hover:bg-muted/20",
                    )}
                  >
                    <div className={cn(
                      "w-12 h-12 rounded-xl flex-shrink-0 overflow-hidden shadow-sm transition-all duration-200",
                      isHighlighted ? "ring-2 ring-emerald-500/40 scale-105" : "bg-muted/50 ring-1 ring-black/10 dark:ring-white/10",
                    )}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-end mb-1.5">
                        ...
                        <div className={cn("text-[13px] font-black tabular-nums ml-4 transition-colors duration-200", isHighlighted ? "text-emerald-500" : "text-emerald-500/80")}>
                      ...
                      <div className="h-1.5 bg-muted/30 rounded-full overflow-hidden">
                        <motion.div
                          ...
                          className={cn("h-full rounded-full shadow-sm transition-opacity duration-200", isHighlighted ? "bg-emerald-400" : "bg-emerald-500")}
                        />
                      </div>
                    </div>
                  </div>
        ...
        {/* Pie chart with sync highlight */}
        <div className={cn("xl:col-span-2 p-4 flex flex-col items-center justify-center rounded-2xl", "bg-muted/10 dark:bg-white/5")}>
          <div className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground mb-6">Revenue Distribution</div>
          <div className="w-full aspect-square max-w-[200px] mb-6">
                  ...
                  >
                    {topProducts.map((p, i) => (
                      <Cell
                        ...
                        style={{
                          opacity: highlightedIdx === null ? 1 : highlightedIdx === i ? 1 : 0.15,
                          transform: highlightedIdx === i ? "scale(1.05)" : "scale(1)",
                          filter: highlightedIdx === i ? "drop-shadow(0 0 8px rgba(74,222,128,0.4))" : "none",
                        }}
                      />
                    ))}
                  </Pie>
            ...
            {topProducts.map((p, i) => {
              ...
              return (
                <div
                  ...
                  onMouseLeave={() => setHighlightedIdx(null)}
                  className={cn(
                    "flex items-center justify-between text-[11px] font-bold uppercase tracking-widest cursor-pointer transition-all duration-200 px-2 py-1 rounded-md",
                  ...
                  <span className="flex items-center gap-3 min-w-0">
                    <span
                      ...
                      style={{
                        ...
                        boxShadow: isHighlighted ? `0 0 8px ${pieColors[i % pieColors.length]}60` : "none",
                      }}
                    />
                    <span className="text-muted-foreground truncate">{p.name}</span>
                  </span>
                  <span className={cn("shrink-0 transition-colors", isHighlighted && "text-emerald-500")}>{displayPct}%</span>
                </div>
              );
            })}
          </div>
